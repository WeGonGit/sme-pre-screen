...
// Path to db.json (inside backend/data)
const file = path.join(__dirname, '..', 'data', 'db.json');
const adapter = new JSONFile(file);

// give lowdb default data
const defaultData = { applications: [] };
const db = new Low(adapter, defaultData);

async function initDB() {
  await db.read();
  // if file was empty, make sure structure exists
...
function decideStatus(docs = {}) {
  ...
  if (docs.kyc && docs.income && docs.businessProof)
    return { status: 'Ready for Appraisal' };

  if (docs.kyc && docs.income && !docs.businessProof)
    return { status: 'Conditional Approval', reason: 'Business proof missing' };

  if (docs.kyc && docs.businessProof && !docs.income)
    return { status: 'On Hold', reason: 'Income proof missing' };
...
module.exports = { createApplication, listApplications, getApplication };
